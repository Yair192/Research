...
for k in range(Config.runs):
    ...
    for i in range(Config.IMU_to_train):
        ...
        x_train_merged_rs = x_train_merged.reshape((Config.N, i + 1, Config.input_channels, Config.samples_to_train))
        x_train_merged_mean = np.mean(x_train_merged_rs, axis=1)
        y_train_merged_rs = y_train_merged.reshape((Config.N, i + 1, Config.input_channels))
        y_train_merged_mean = np.mean(y_train_merged_rs, axis=1)
        ...
        # Data loaders
        X_train, X_val, y_train, y_val = train_test_split(x_train_merged_mean, y_train_merged_mean, test_size=0.2,
                                                          random_state=seed)
        ...
        for epoch in range(CNNet.epochs):
            ...
            if epoch == 4:
                ...
                if not os.path.isdir(f'/home/ystolero/Documents/Research/Simulation/checkpoints_avg_on_data/run_{k}'):
                    os.mkdir(f'/home/ystolero/Documents/Research/Simulation/checkpoints_avg_on_data/run_{k}')
                torch.save(state,

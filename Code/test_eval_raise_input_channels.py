...
for k in range(Config.runs):
    j = 0
    in_ch = 3
    for i in range(0,Config.IMU_to_train):

    # for i in Config.imu_to_train:
        model = CNNet.CNN1DRaiseInput(in_ch).to(device)
...
plt.plot(Config.t_arr, L1_RA_X_test, label=r"$\mu[t]$")
plt.scatter([10,30], [0.000765253682866538,0.00034117347663621515] , label = "1 IMU")
plt.scatter([10,30], [0.0007065650308685782, 0.00048517708155385963], label = "2 IMUs")
plt.scatter([10,30], [0.00023245486936365804, 0.0007914460631416943], label = "3 IMUs")
plt.scatter([10,30], [0.000552832180656513, 0.0004696124229218614], label = "4 IMUs")
